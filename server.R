...
server <- function(input, output) {
    ...
    isolate({
      ...
      if(is.null(input$custom_data) == F){
        ...
        ### Custom dataset read
            ...
            df <- data.table::fread(input$custom_data$datapath, sep = input$sep_custom) %>% as.data.frame() %>%
            dplyr::select(1:10)
            colnames(df) <- c("patient_id", "gene_id","exon_id","Chromosome", "Start_Position", "End_Position", 
  ...
  informative_merged_df <- reactive({
    ...
    isolate({
        ...
        if(input$analysis_type == "test"){
          ...
          df <- panel_tester(merged_dataset, group_by = input$mutation_group,
                             info_metric = input$metric_type, min_pts = input$min_pts,
                             custom_panel = custom_panel, keep_order = F)
          
        } else {
          df <- panel_finder(merged_dataset, group_by = input$mutation_group,
                             info_metric = input$metric_type, min_pts = input$min_pts,
          ...
          independent_sets <- mutations_raw_data() %>% 
            ...
            group_by(mutation_id) %>% summarise(n_cohort = length(unique(cohort)))
          
          df <- df %>% left_join(independent_sets, by = "mutation_id") %>%
            droplevels()
  ...
  suggested_genes <- reactive({
    ...
    isolate({
      ...
      if(input$analysis_type == "optimal" | max(informative_merged_df() %>% dplyr::select(percent_tot)) >= 99.9){
        ...
        optimal_panel <- informative_merged_df() %>% distinct(mutation_id) %>% unlist() %>% as.character()
        ...
        df <- panel_finder(new_data_list, group_by = input$mutation_group,
                           info_metric = input$metric_type, min_pts = input$min_pts,
                           custom_panel = NULL) %>%
          mutate(percent_remaining = round(percent_tot, 2),
  ...
  output$sup_genes_message <- renderText({
    ...
    remaining_pts <- mutations_raw_data() %>% 
      filter(!patient_id %in% diag_pts) %>%
      distinct(patient_id) %>% nrow() %>% as.numeric()
  ...
  table_mut_stat_df <- reactive({
    ...
    isolate({
      ...
      if(is.null(input$panel_list) | input$analysis_type == "optimal"){
        custom_panel = mutations_raw_data() %>% dplyr::select(mutation_id= input$mutation_group) %>% unlist() %>% as.character()
      } else {
        custom_panel <- unlist(panel_df() %>% dplyr::select(mutation_id)) %>% as.character()
      }
      ...
      df <- mutations_raw_data() %>% 
        ...
        ungroup() %>%
        dplyr::select(mutation_id, unique_pts,percent_total, unique_mut, mut_types, 
  ...
  table_des_df <- reactive({
    ...
    isolate({
      ...
      mutations <- informative_merged_df() %>% dplyr::select(mutation_id) %>% unlist() %>% as.character()
      ...
      df_stat <- df %>%
       ...
       group_by(patient_id) %>%
        ...
        rbind(data.frame(n_mutations = 0,
                         exact =   length(setdiff(unique(df$patient_id),
                                                  unique(df$patient_id[df$mutation_id %in% mutations])) ),
                         at_least = length(setdiff(unique(df$patient_id),
                                                   unique(df$patient_id[df$mutation_id %in% mutations])) ))) %>%
        arrange((n_mutations))
    ...
    isolate({
      ...
      if(length(names(dataset_list)) == 1){
        ...
        if(input$analysis_type == "test"){
          ...
          df <- panel_tester(dataset_list, group_by = input$mutation_group,
                             info_metric = input$metric_type, min_pts = input$min_pts,
                             custom_panel = custom_panel, keep_order = F)
        ...
        df <- panel_finder(dataset_list, group_by = input$mutation_group,
                           info_metric = input$metric_type, min_pts = input$min_pts,
                           custom_panel = custom_panel )
        }
      }
  ...
  mutation_heat_df <- reactive({
    ...
    isolate({
      ...
      optimal_genes <- informative_merged_df() %>% distinct(mutation_id) %>% unlist() %>% as.character()
      ...
      no_mut_patients <- df %>% filter(optimal_mutation == F) %>% 
        distinct(patient_id) %>% unlist() %>% as.character()
      mut_patients <- df %>% filter(optimal_mutation == T) %>% 
        distinct(patient_id) %>% unlist() %>% as.character()
  ...
  graph_length_df <- reactive({
    ...
    isolate({
      ...
      base_pts <- panel_tester(data_list, group_by = input$mutation_group,
      ...
      base_pts_kb <- panel_tester(data_list, group_by = input$mutation_group,
      ...
      ### PIO: n pts
      PIO_pts <- panel_finder(data_list, group_by = input$mutation_group,
                              info_metric = "unique_pts", min_pts = input$min_pts,
                              custom_panel = NULL) %>%
      ...
      ### PIO: pts per kb
      PIO_pts_per_kb <- panel_finder(data_list, group_by = input$mutation_group,
                              info_metric = "pts_per_kb", min_pts = input$min_pts,
                              custom_panel = NULL) %>%
      ...
      df <- rbind(base_pts, base_pts_kb, PIO_pts, PIO_pts_per_kb) %>%
        mutate(group = factor(group, levels = c("unique_pts","pts_per_kb",
  ...
  output$graph_informative_merged <- renderPlot({
    ...
    data_plot <- informative_merged_df() %>%
        filter(percent_tot <= input$max_freq,
    ...
    data_anno <- data.frame(
      ...
      y = 0.3,
      tot_pts = paste0(round(max(data_plot %>% dplyr::select(percent_tot) )*100,2),"% pts diagnosed \n with ", 
    ...
    n_pts <- mutations_raw_data() %>%
      distinct(patient_id) %>% unlist() %>% length()
  ...
  #  test <- panel_data_merged %>% filter(Disease == "AML")
    ...
    data_length <- data_plot %>%
      ...
      ggplot(data = data_plot,  aes(x = forcats::fct_reorder(mutation_id,percent_tot), 
                                                          y = percent_tot, group = 1)) +
      geom_point(aes(color = as.factor(n_cohort))) +
      geom_line(aes(color = as.factor(n_cohort))) +
      scale_color_viridis_d()+
      geom_line(data = filter(data_length, mutation_id %in% plot_genes ),
                  aes(x = forcats::fct_reorder(mutation_id,percent_tot), 
                      y = percent_length, group = 1), color = "red") +
      geom_text_repel(data = data_anno, aes(x = x, y = y , label = tot_pts),size = 6)+
      custom_theme +
      labs(title = paste0("Most informative genes in ",if_else(is.null(input$custom_data),input$disease,"Custom dataset"),
                          " (n= ",n_pts," pts)"),
           x = "", group = "test",
  ...
  output$graph_informative <- renderPlot({
    ...
    for(study in unique(informative_df() %>% dplyr::select(cohort) %>% unlist() )){
      ...
      data_plot <- informative_df() %>% filter(cohort == study) %>% droplevels()
      ...
      plot_list[[study]] <-  ggplot(data = data_plot,  aes(x = forcats::fct_reorder(mutation_id,percent_tot), 
                                                           y = percent_tot, group = 1)) +
        geom_point() +
        geom_line() +
        scale_color_viridis_d()+
        ...
        geom_text_repel(data = data_anno, aes(x = x, y = y , label = tot_pts),size = 6)+
        custom_theme +
        labs(title = paste0(study," (n= ",n_pts," pts)"),
             color = "Cohorts (n)", x = "") +
        ylim(0,1) +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold",
                                         size = input$x_size),
  ...
  }, height = graph_height) #Renderplot
...
output$graph_mut_freq <- renderPlot({
  ...
  isolate({
    ...
    top_n_genes <-  mutation_des_merged_df() %>% 
        group_by(mutation_id) %>%
        summarise(tot_pts = sum(percent_tot),
                  tot_count = sum(count)) %>%
        arrange(desc(tot_pts)) %>% dplyr::slice(1:input$max_genes) %>% 
        distinct(mutation_id) %>% unlist() %>% as.character()
    ...
    data_plot %>%
      ...
      labs(title = paste0("Most frequent mutations in ",title,
...
output$graph_heat <- renderPlot({
  ...
  isolate({
    ...
    mutation_heat_df() %>% 
      ...
      labs(title = paste0("Mutations distribution across ",title," patients"),
           x = "Patients", y = "") +
...
}, height = graph_height_2) #Renderplot
...
output$graph_length <- renderPlot({
  ...
  isolate({
    ...
    graph_length_df() %>%
      ...
      scale_color_viridis_d()+
      custom_theme +
      labs(title = paste0("Panel length comparison in ",input$disease,
                          " (n= ",n_pts," pts)"),
