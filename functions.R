...
#' 
#' @param group_by Either gene_id or exon_id
#' @param info_metric Either unique_pts or pts_per_kb

panel_finder <- function(data_list, group_by = "gene_id", info_metric = "unique_pts",
                         custom_panel = NULL, min_pts = 1 ){
  ...
  for(datasets in names(data_list)){
   ...
   data_mutation <- data_mutation %>% filter(mutation_id %in% custom_panel)
   ...
   data_mutation <- data_mutation %>%
    ...
    while(n_added > 0 ){
      ...
      results_list[[last(genes_tested)]] <- data.frame(
        n_pts = n_added,
      ...
      genes_to_add <- append(genes_to_add, as.character(next_gene[1,2]))
    ...
    if(group_by == "gene_id"){
      results_df_list[[datasets]] <- results_df_list[[datasets]] %>%
        left_join(distinct(dplyr::select(ordered_mutations,mutation_id, length), mutation_id, .keep_all = T), 
                  by = c("mutation_id")) %>%
        mutate(pts_per_kb = round(n_pts/length,4))
      
    } else {
      results_df_list[[datasets]] <- results_df_list[[datasets]] %>%
        left_join(dplyr::select(ordered_mutations,mutation_id, length), by = c("mutation_id")) %>%
        mutate(pts_per_kb = round(n_pts/length,4))
...
panel_tester <- function(data_list, group_by = "gene_id", info_metric = "unique_pts",
                         custom_panel = NULL, keep_order = T, min_pts = 1){
  ...
  for(datasets in names(data_list)){
    ...
    for(mutations in ordered_mutations){
      ...
      new_pts <- data_mutation %>%
        filter(mutation_id == mutations & !patient_id %in% pts_tested) %>%
        distinct(patient_id) %>% unlist() %>% as.character()
      
      n_added <- length(new_pts)
      
      results_list[[mutations]] <- data.frame(
        n_pts = n_added,
    ...
    if(group_by == "gene_id"){
      results_df_list[[datasets]] <- results_df_list[[datasets]] %>%
        left_join(distinct(dplyr::select(data_mutation,mutation_id, length), 
                           mutation_id, .keep_all = T), 
                  by = c("mutation_id")) %>%
        mutate(pts_per_kb = round(n_pts/length,4))
